...
void web::http::Request::ParseQueryString() {
  queryData = web::http::parse_form_urlenconded(this->queryString);
}
void web::http::Request::ParseFormString() {
  ...
  if (cstf::string_contains(contentType, "application/x-www-form-urlencoded")) {
    this->formData = web::http::parse_form_urlenconded(this->formString);
    return;
  }
}
