import * as cmenu from "./context-menu.js"
import { fetchText } from "./connection.js"
...
domController.register(`a[href]`, function (e) {
  ...
  if (url.includes("?")) url += "&partial=true";
	...
	if(domController.onPageRemoved)
		domController.onPageRemoved();

	if(domController.pageLoops && domController.pageLoops.length > 0){
		for(let i = 0; i < domController.pageLoops.length; i++)
			clearInterval(domController.pageLoops[i]);
	}
...
export { domController, enterDoSubmit };