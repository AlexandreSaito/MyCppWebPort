import * as Main from "/dist/js-modules/main.js"
import * as CtxMenu from "/dist/js-modules/context-menu.js"
...
Main.enterDoSubmit("#input-chat", "#send-chat", true);

const chatContextMenu = new CtxMenu.ContextMenu("chat!");
registerChatItens();

Main.domController.register(".user", selectChatUser);
...
function collapseSelectedUser() {
  ...
  if (chatUser.dataset.currentUserName && chatUser.dataset.collapsed == "false") {
...
function selectChatUser(e) {
  ...
  let rs = getComputedStyle(Main.root);
  let closingTime = rs
...
function registerChatWebsocket(){
  ...
  chatSocket.onopen = function(e) { 
		chatSocket.send(createWebsocketMessage({userName: userName}, {op: 1})); 
	}
...
function registerChatItens() {
  let hideItem = new CtxMenu.ContextMenuItem({
    name: "Esconder chat.",
    action: () => {
  ...
  let userNameItem = new CtxMenu.ContextMenuItem({
    name: "",
    onOpen: (item) => {
      item.querySelector(
        ".item-name"
  ...
  let teste = new CtxMenu.ContextMenuItem({
    name: "teste",
